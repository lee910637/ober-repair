...
# ---------------------------------------------------------------------------
# 0. 機型 (Models) —— 歐伯維修 首頁/次頁 的機型清單，歸屬 A/N/E 系列
# ---------------------------------------------------------------------------
ws0 = wb.active
ws0.title = "機型"
models_headers = ["Model_ID", "型號名稱", "系列", "圖片", "是否已建置", "說明"]
models_rows = [
    ["17108A-5-120", "17108A-5-120 電池化成測試器", "A", "images/17108A-5-120.jpg", "是", "電池化成測試器，120A 一體機。"],
    # 未來新增機型往下加列即可，例如：
    # ["17108N-x-xxx", "17108N-x-xxx xxx", "N", "images/xxx.jpg", "否", "..."],
]
write_sheet(ws0, models_headers, models_rows)

# ---------------------------------------------------------------------------
# 1. 異常資訊 (Faults) —— 對應手冊第 3 章「全機保護訊息」表 + 第 5 章查修方向
#    每筆都要標明 Model_ID，歸屬到「機型」表裡的某一台機器。
# ---------------------------------------------------------------------------
ws1 = wb.create_sheet("異常資訊")
faults_headers = ["Fault_ID", "Model_ID", "名稱", "關鍵字", "類別", "優先等級", "是否已建置", "起始節點", "手冊章節", "說明"]
faults_rows = [
    ["F_CALI", "17108A-5-120", "Calibration Error 校正值異常", "校正值異常,Slope,Calibration Out of Range", "校正", "高", "否", "", "4.8節", "校正值 Slope>20%，判定為 Calibration Out of Range。"],
    ["F_OCP", "17108A-5-120", "OCP／OVP 過電流過電壓保護", "OCP,OVP,過電流,過電壓,Input Voltage Protection", "保護", "高", "是", "OCP_1", "4.1節", "Iout>134A 或 Vout>5.8V 或 Vin>14V 時觸發。"],
    ["F_OTP", "17108A-5-120", "OTP 單機環溫過溫保護", "OTP,過溫,溫度異常", "溫度", "高", "否", "", "4.2節", "單機環溫 >100°C 觸發過溫保護。"],
    ["F_CONTACT", "17108A-5-120", "接觸錯誤／感應異常保護", "接觸錯誤,感應異常,Vmi,Vmo", "量測", "高", "否", "", "4.3節", "ABS(Vmi-Vmo)/I 超出設定值，Sense/Drive 線路或量測電路異常。"],
    ["F_FAN", "17108A-5-120", "FAN 風扇異常保護", "Fan,風扇,異常,鎖轉,轉速", "散熱", "中", "是", "FAN_1", "4.5節", "風扇電源異常或風扇本體損壞造成 Fan Lock 保護。"],
    ["F_NET", "17108A-5-120", "網路通訊異常", "網路,LAN,IP,通訊異常", "通訊", "中", "否", "", "4.4節", "PC 與單機之間 LAN 通訊異常，無法連線。"],
    ["F_SYSCFG", "17108A-5-120", "System Config Error", "System Config,模組錯誤,12V,13V", "控制", "中", "否", "", "4.6節", "前級轉換器(AC-DC)異常或通道數/Slot數設定錯誤。"],
    ["F_ADC", "17108A-5-120", "ADC Error 讀值異常", "ADC,讀值異常,固定0V,固定0A", "量測", "高", "否", "", "4.7節", "電壓/電流讀值固定不跳動，疑似 ADC 或 A 板異常。"],
    ["F_NOOUTPUT", "17108A-5-120", "無輸出電壓，DVM無讀值", "無輸出電壓,DVM無讀值,CV4V 0V 0V", "校驗證查修", "高", "是", "NOV_1", "5.2.1節", "執行 8000 程式校驗證時，設定電壓後完全無輸出，內部讀值與 DVM 實測值皆為 0V。"],
]
